...
function promedioResultadosTest(resultadosTest) {
  ...
  for(let i = 0; i < resultadosTest.length; i++){
   sumandotest = sumandotest + resultadosTest[i];   
  }
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  var numeroemp9 = n.toString();
  if(numeroemp9.charAt(0) === "9"){
    return true;
  }
  return false;
}
...
function breakStatement(numero) {
  ...
  // Tu código:
  var arraynuevo = [];
  var sumanueva = numero;
  for(var i= 0; i<10; i++) {
    sumanueva = sumanueva + 2;
    if(sumanueva === i) {
      break;}
    else {
      arraynuevo.push(sumanueva);
    }
  }
  if(i < 10) {
